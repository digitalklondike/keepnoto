...
const dropdownMenuItemClassName = cn(
  "flex h-[var(--size-48)] w-full cursor-pointer items-center gap-[var(--space-8)] rounded-[var(--radius-12)] px-[var(--space-12)] type-16 outline-none transition-[background-color,color,opacity] duration-150 ease-[cubic-bezier(0.23,1,0.32,1)]",
...
export function DropdownMenuContent({ className, contentClassName, ...props }: DropdownMenuContentProps) {
  return (
    <BaseDropdownMenuContent
      {...props}
      className={cn(
        "flex w-[var(--dropdown-width)] origin-(--transform-origin) flex-col gap-[var(--space-4)] rounded-[var(--radius-20)] !border-0 bg-[var(--popover-surface)] p-[var(--space-8)] text-[var(--content-primary)] !shadow-[var(--shadow-panel)] !ring-0 backdrop-blur-[var(--blur-panel)] transition-[opacity,transform] duration-200 ease-[cubic-bezier(0.23,1,0.32,1)] data-[ending-style]:scale-[0.98] data-[ending-style]:opacity-0 data-[starting-style]:scale-[0.98] data-[starting-style]:opacity-0",
        contentClassName,
        className
      )}
    />
