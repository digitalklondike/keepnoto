...
export const TextField = React.forwardRef<HTMLInputElement, TextFieldProps>(function TextField({
  ...
  return (
    <label
      data-state={visualState}
      className={cn(
        "group flex h-[var(--size-48)] w-[var(--search-width)] items-center rounded-[var(--radius-round)] px-[var(--space-16)] text-[var(--content-primary)] backdrop-blur-[var(--blur-soft)] transition-[background-color,opacity,box-shadow]",
...
export function Tag({ add, visualState = "default", className, children, ...props }: TagProps) {
  ...
  const tag = (
    <button
      ...
      className={cn(
        "inline-flex h-[var(--size-24)] min-w-0 max-w-[var(--tag-max-width)] items-center rounded-[var(--radius-round)] border border-transparent bg-[var(--tag-fill)] pl-[var(--space-12)] pr-[var(--space-12)] py-[var(--space-4)] type-12-semibold text-[var(--content-primary)] transition-[transform,background-color,border-color,color,opacity]",
